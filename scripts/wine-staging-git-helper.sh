...
get_sieved_git_commit_log()
{
	...
	if (($# == 4)); then
		__git_log="$( git log --format='%H,%P' --reverse --all --after "${start_date}" --before "${end_date}" || die "${FUNCNAME[0]}(): git log" )"
	elif (($# == 3)); then
		__git_log="$( git log --format='%H,%P' --reverse --all --after "${start_date}" || die "${FUNCNAME[0]}(): git log" )"
	else
		__git_log="$( git log --format='%H,%P' --reverse --all || die "${FUNCNAME[0]}(): git log" )"
	fi
	if [[ ! -z "${__git_dir}" ]]; then
		popd >/dev/null || die "${FUNCNAME[0]}(): popd"
	fi
...
find_closest_wine_commit()
{
	...
	local -r wine_git_date_roffset="-4 weeks" wine_git_date_foffset="+1 week"
